...
<root>
  ...
  <items>
    ...
    <item app="openssh" version="__OPENSSH_VERSION__" />
...
sed -i -e "s|__OPENSSH_VERSION__|${openssh_version}|g" build_request.xml
...
do
    ...
    ssh -V
    echo "Pattern 1"
    ssh -vv -F ${OPENSHIFT_DATA_DIR}/.ssh/config -i ${OPENSHIFT_DATA_DIR}/.ssh/id_rsa ${user_fqdn} pwd 2>&1 | tee -a ${OPENSHIFT_LOG_DIR}/install.log
    echo "Pattern 2"
    ssh -vv -F ${OPENSHIFT_DATA_DIR}/.ssh/config ${user_fqdn} pwd 2>&1 | tee -a ${OPENSHIFT_LOG_DIR}/install.log
