...
export TZ=JST-9
echo `date +%Y/%m/%d" "%H:%M:%S` Install STEP 7 Start | tee -a ${OPENSHIFT_LOG_DIR}/install.log
...
echo `date +%Y/%m/%d" "%H:%M:%S` Install STEP 7 Finish | tee -a ${OPENSHIFT_LOG_DIR}/install.log
