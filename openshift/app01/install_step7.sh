...
CC="ccache gcc" CFLAGS="-O2 -march=native" CXXFLAGS="-O2 -march=native" \
./configure \
...
echo $'\n'$(date +%Y/%m/%d" "%H:%M:%S) '***** make *****' $'\n'$'\n'>> ${OPENSHIFT_LOG_DIR}/install_libmemcached.log
time make -j$(grep -c -e processor /proc/cpuinfo) -l3 2>&1 | tee -a ${OPENSHIFT_LOG_DIR}/install_libmemcached.log
