...
export TZ=JST-9
echo `date +%Y/%m/%d" "%H:%M:%S` Install STEP 12 Start | tee -a ${OPENSHIFT_LOG_DIR}/install.log
...
time make -j2 CFLAGS="-O3 -march=native -pipe" CXXFLAGS="-O3 -march=native -pipe" >${OPENSHIFT_LOG_DIR}/delegate.make.log 2>&1
...
echo `date +%Y/%m/%d" "%H:%M:%S` Install STEP 12 Finish | tee -a ${OPENSHIFT_LOG_DIR}/install.log
